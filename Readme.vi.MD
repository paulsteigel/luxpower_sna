...
- **Tự động tìm dongle** — nhấn nút *Scan Dongle IP*, ESP32 quét toàn bộ /24 subnet và tìm dongle tự động
...
> **Quan trọng:** Thêm block `on_boot` để push giá trị đã lưu flash vào hub sau mỗi lần reboot. Nếu không, hub sẽ không nhận được serial number cho đến khi người dùng chỉnh sửa lại.
...
Nếu không biết IP của dongle, dùng nút **Scan Dongle IP**. ESP32 sẽ quét 254 địa chỉ trên /24 subnet của chính nó, kết nối thử từng địa chỉ trên port đã cấu hình (mặc định 8000), và tự kết nối khi tìm thấy.
...
5. Khi tìm thấy dongle:
   ...
   - Component tự kết nối ngay lập tức
6. Sao chép IP từ **Scan Status** và dán vào **Inverter Host** để lưu cho các lần reboot sau.
7. Nếu không tìm thấy: `scan_status_text` hiển thị `Not found`.
...
|---------|---------|
| `Scanning...` | Đang quét |
| `Found: 192.168.x.x` | Đã tìm thấy; sao chép IP vào Inverter Host để lưu |
...
4. Kiểm tra **Scan Status** — nếu hiện `Found: 192.168.x.x`, component đã kết nối thành công
5. Sao chép IP tìm được và dán vào **Inverter Host** để lưu cho các lần reboot sau
6. Sensor và điều khiển hoạt động ngay sau khi scan thành công
...
| Serial trống sau reboot | Thiếu block `on_boot` | Thêm block `on_boot` như hướng dẫn trong phần Cấu hình runtime |
